...
!
      REAL,DIMENSION(IMS:IME,JMS:JME,1:lm),INTENT(IN):: &
       dz,exner,OMGALF,phmid,rr,t,th,U,V, w_tot
!
      REAL,DIMENSION(IMS:IME,JMS:JME,1:lm-1),INTENT(IN):: PSGDT   !pa/s,vertical mass flux
...
! 
      ...
      REAL,DIMENSION(IMS:IME,JMS:JME) :: PRATEC
!
...
!
      LOGICAL,DIMENSION(IMS:IME,JMS:JME),INTENT(INOUT):: &
       ...
       REAL, OPTIONAL, INTENT(IN) ::    PGCON,sas_mass_flux  &
                                       ,shal_pgcon,shalconv
...
!                        +V(I,J-1,KFLIP)+V(I-1,J-1,KFLIP))*0.25
        ...
        !   clw(1,K,2) = QI(I,J,KFLIP)+QS(I,J,KFLIP)+QG(I,J,KFLIP)   ! ICE
           ...
           VVEL(1,K)    = VVEL(1,k)*0.001       !! kpa/s  

           !VVEL(1,k)  = -0.001*RR(I,J,KFLIP)*G*w_tot(I,J,KFLIP) 
           tmp2  = -0.001*RR(I,J,KFLIP)*G99*w_tot(I,J,KFLIP) 
           tmp3=0.0
!         if(kflip <= lm-1)tmp3  = 0.001*PSGDT(I,J,KFLIP)
!   PSGDT is LM-1 in z dimension 
           if(kflip-1 <= lm-1 .and. kflip-1 >= 1 )tmp3  = 0.001*PSGDT(I,J,KFLIP-1)

             if (lpr .and. i == 10 .and. j == 10 .and. k .le. 50) &
                              write(0,*)k,VVEL(1,K), tmp2, tmp3
          ENDDO
             !write(0,*)'i=,j=',i,j
