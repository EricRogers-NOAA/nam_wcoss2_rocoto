...
!+---+-----------------------------------------------------------------+
      ...
      CALL wrf_debug(100, 'CREATING MICROPHYSICS LOOKUP TABLES ... ')
...
!..Conversion of some ice mass into snow category.
      ...
      CALL wrf_debug(100, ' ... DONE microphysical lookup tables')
...
!+---+-----------------------------------------------------------------+
      SUBROUTINE mp_gt_driver(qv, qc, qr, qi, qs, qg, ni, nr, &
                              ...
                              its,ite, jts,jte, kts,kte,d_ss,mprates)  ! tile dims
...
!..Subroutine arguments
      ...
      REAL, DIMENSION(ims:ime, jms:jme,kms:kme,d_ss) :: mprates 
      ...
      INTEGER, OPTIONAL, INTENT(IN) :: has_reqc, has_reqi, has_reqs
...
!aligo
      REAL, DIMENSION(kts:kte):: vtcik,vtcsk,vtcrk,vtcgk,vtcink, &
              vtcrnk,qvten1d,qcten1d,qrten1d,qiten1d,qsten1d,    &
              qgten1d,niten1d,nrten1d,rho1d,n0_r1d
!aligo
...
!+---+
      ...
      i_loop:  do i = i_start, i_end
         ...
         call mp_thompson(qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d, &
                      ...
                      kts, kte, dt, i, j,vtcik,vtcsk,vtcgk,vtcrk,   &
                      vtcink,vtcrnk,qvten1d,qcten1d,qiten1d,qrten1d,&
                      qsten1d,qgten1d,niten1d,nrten1d,n0_r1d)
         ...
         do k = kts, kte
            ...
            ni(i,j,kflip) = ni1d(k)
            nr(i,j,kflip) = nr1d(k)
            th(i,j,kflip) = t1d(k)/pii(i,j,kflip)
            IF(D_SS.EQ.1)THEN
               mprates(I,J,kflip,1)=0.
            ELSE
               mprates(I,J,kflip,1)=vtcsk(k)
               mprates(I,J,kflip,2)=vtcrk(k)
               mprates(I,J,kflip,3)=vtcik(k)
               mprates(I,J,kflip,4)=vtcgk(k)
               mprates(I,J,kflip,5)=vtcrnk(k)
               mprates(I,J,kflip,6)=vtcink(k)
               mprates(I,J,kflip,7)=mprates(I,J,kflip,7)+qvten1d(k)
               mprates(I,J,kflip,8)=mprates(I,J,kflip,8)+qcten1d(k)
               mprates(I,J,kflip,9)=mprates(I,J,kflip,9)+qiten1d(k)
               mprates(I,J,kflip,10)=mprates(I,J,kflip,10)+qrten1d(k)
               mprates(I,J,kflip,11)=mprates(I,J,kflip,11)+qsten1d(k)
               mprates(I,J,kflip,12)=mprates(I,J,kflip,12)+qgten1d(k)
               mprates(I,J,kflip,13)=mprates(I,J,kflip,13)+niten1d(k)
               mprates(I,J,kflip,14)=mprates(I,J,kflip,14)+nrten1d(k)
               mprates(I,J,kflip,15)=n0_r1d(k)
            ENDIF
            if (qc1d(k) .gt. 100.E-3) then
             write(mp_debug,*) 'WARNING, BAD qc ',qc1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (qc1d(k) .gt. qc_max) then
             imax_qc = i
             jmax_qc = j
             kmax_qc = k
             qc_max = qc1d(k)
            ...
            if (qr1d(k) .gt. 100.E-3) then
             write(mp_debug,*) 'WARNING, BAD qr ',qr1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (qr1d(k) .gt. qr_max) then
             imax_qr = i
             jmax_qr = j
             kmax_qr = k
             qr_max = qr1d(k)
            ...
            if (nr1d(k) .gt. 1.E12) then
             write(mp_debug,*) 'WARNING, BAD Nr ',nr1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (nr1d(k) .gt. nr_max) then
             imax_nr = i
             jmax_nr = j
             kmax_nr = k
             nr_max = nr1d(k)
            ...
            if (qs1d(k) .gt. 100.E-3) then
             write(mp_debug,*) 'WARNING, BAD qs ',qs1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (qs1d(k) .gt. qs_max) then
             imax_qs = i
             jmax_qs = j
             kmax_qs = k
             qs_max = qs1d(k)
            ...
            if (qi1d(k) .gt. 100.E-3) then
             write(mp_debug,*) 'WARNING, BAD qi ',qi1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (qi1d(k) .gt. qi_max) then
             imax_qi = i
             jmax_qi = j
             kmax_qi = k
             qi_max = qi1d(k)
            ...
            if (qg1d(k) .gt. 100.E-3) then
             write(mp_debug,*) 'WARNING, BAD qg ',qg1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (qg1d(k) .gt. qg_max) then
             imax_qg = i
             jmax_qg = j
             kmax_qg = k
             qg_max = qg1d(k)
            ...
            if (ni1d(k) .gt. 1.E18) then
             write(mp_debug,*) 'WARNING, BAD Ni ',ni1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (ni1d(k) .gt. ni_max) then
             imax_ni = i
             jmax_ni = j
             kmax_ni = k
             ni_max = ni1d(k)
            ...
            if (qv1d(k) .gt. 100.E-3) then
             write(mp_debug,*) 'WARNING, BAD Qv ',qv1d(k),' at i,j,k=', i,j,k, itimestep
             CALL wrf_debug(0, mp_debug)
            elseif (qv1d(k) .lt. 0.0) then
         ...
         IF ( PRESENT (diagflag) .AND. PRESENT(refl_10cm) ) THEN
...
!
      subroutine mp_thompson (qv1d, qc1d, qi1d, qr1d, qs1d, qg1d, ni1d, &
                          nr1d, t1d, p1d, dzq, &
                          pptrain, pptsnow, pptgraul, pptice, &
                          kts, kte, dt, ii, jj,vtcik,vtcsk,vtcgk, &
                          vtcrk,vtcink,vtcrnk,qvten1d,qcten1d,    &
                          qiten1d,qrten1d,qsten1d,qgten1d,niten1d, &
                          nrten1d,n0_r1d)
...
!..Sub arguments
      ...
      REAL, DIMENSION(kts:kte), INTENT(INOUT):: &
                          ...
                          nr1d, t1d, p1d
!aligo
      REAL, DIMENSION(kts:kte), INTENT(OUT):: vtcik,vtcsk,vtcgk,&
       vtcrk,vtcink,vtcrnk,qvten1d,qcten1d,qiten1d,qrten1d,&
       qsten1d,qgten1d,niten1d,nrten1d,n0_r1d
!aligo
...
!+---+-----------------------------------------------------------------+
      if( debug_flag) then
         write(mp_debug,*) 'DEBUG-GT:   pres,  temp,  qv,           rc,            ri,            rr,            rs,            rg,            Ni,            Nr'
         CALL wrf_debug(0, mp_debug)
         do k = kts, kte
            write(mp_debug, '(3(a,i3),a,2x,f7.2,1x,f6.2,2x,f7.4,7(1x,e13.6,1x))') &
                       '(',ii,',',jj,',',k,')',                         &
                        pres(k)*0.01, temp(k)-273.15, qv(k)*1000.,      &
                        rc(k), ri(k), rr(k), rs(k), rg(k), ni(k), nr(k)
            CALL wrf_debug(0, mp_debug)
         enddo
      endif
!+---+-----------------------------------------------------------------+
...
!aligo
         qvten1d(k)=qvten(k)*DT
         qcten1d(k)=qcten(k)*DT
         qiten1d(k)=qiten(k)*DT
         qrten1d(k)=qrten(k)*DT
         qsten1d(k)=qsten(k)*DT
         qgten1d(k)=qgten(k)*DT
         niten1d(k)=niten(k)*DT
         nrten1d(k)=nrten(k)*DT
         n0_r1d(k)=N0_r(k)
!aligo
      enddo
...
!+---+-----------------------------------------------------------------+
      ...
      do k = kte-1, kts, -1
         if ( (temp(k).gt.273.15) .and. L_qr(k)                         &
            ...
            k_0 = MAX(k+1, k_0)
!..Temporarily disable computing effects of melting ice so the routine
!.. can run every timestep. Later, we can pass a flag for history
!.. output writing time, and enable this only at those times.
!           melti=.true.
            goto 195
         endif
      enddo
...
!+---+-----------------------------------------------------------------+
      ...
      SUBROUTINE cal_cldfra3(CLDFRA, qv, qc, qi, qs, f_ni, ni,          &
...
!
     ...
     &                      its,ite, jts,jte, kts,kte
      ...
      REAL, DIMENSION(ims:ime,jms:jme,kms:kme), INTENT(IN):: qv,p,t,rho
      REAL, DIMENSION(ims:ime,jms:jme,kms:kme), INTENT(INOUT)::         &
                                                            qc,qi,qs,ni

      REAL, DIMENSION(ims:ime,jms:jme), INTENT(IN):: XLAND

      REAL, DIMENSION(ims:ime,jms:jme,kms:kme), INTENT(OUT):: cldfra
...
!..Local vars.
      REAL:: RH_00L, RH_00O, RH_00, RHI_max,                            &
             cni1, cni2, cni3, rbmi, lami, nci, xDi

      REAL, DIMENSION(ims:ime,jms:jme,kms:kme):: qvsat
...
!+---+
      ...
      if (f_ni) then
         cni1 = am_i*cig(2)*oig1
         cni2 = cig(1)*oig2/am_i
         cni3 = bm_i + mu_i + 1.
         rbmi = 1./bm_i
      endif
...
!.. RH over ocean required as compared to over land.
      ...
      !write(*,*) "===== kms, kme ====", kms, kme
      !write(*,*) "===== kts, kte ====", kts, kte
      !write(*,*) "===== jts, jte ====", jts, jte
      !write(*,*) "===== its, ite ====", its, ite
      ...
      DO i = its,ite
         ...
         !write(*,*) "===== j, k, i ====", j, k, i
         !write(*,*) "===== qv(i,j,k) ====", qv(i,j,k)
         !write(*,*) "===== qc(i,j,k) ====", qc(i,j,k)
         !write(*,*) "===== qi(i,j,k) ====", qi(i,j,k)
         !write(*,*) "===== qs(i,j,k) ====", qs(i,j,k)


         if (qc(i,j,k)+qi(i,j,k)+qs(i,j,k) .gt. 1.E-7) then
            CLDFRA(I,J,K) = 1.0
            qvsat(i,j,k) = qv(i,j,k)

            !write(*,*) "===== CLDFRA(I,J,K) ====", CLDFRA(I,J,K)
            ...
            qvsw = rslf(P(i,j,k), TK)
            qvsi = rsif(P(i,j,k), TK)
            ...
            elseif (tc .lt. -30.0) then
               ...
               qvsat(i,j,k) = qvsw - (qvsw-qvsi)*(-12.0-tc)/(-12.0+30.)
            endif

            !write(*,*) "===== qvsat(i,j,k) ====", qvsat(i,j,k)
            !write(*,*) "===== qv(i,j,k) ====", qv(i,j,k)

            RHUM = qv(i,j,k)/qvsat(i,j,k)
            ...
            if (tc .ge. -12.0) then
               RHUM = MIN(0.999, RHUM)
               CLDFRA(I,J,K) = MAX(0.0, 1.0-SQRT((1.0-RHUM)/(1.-RH_00)))
            elseif (tc.lt.-12..and.tc.gt.-70. .and. RHUM.gt.RH_00O) then
               RHI_max = MAX(RHUM+1.E-6, qvsw/qvsi)
               CLDFRA(I,J,K) = MAX(0., ((RH_00O-RHUM)/(RH_00O-RHI_max)) &
     &                                *((RH_00O-RHUM)/(RH_00O-RHI_max)))
            endif

            CLDFRA(I,J,K) = MIN(0.95, CLDFRA(I,J,K))
...
!        endif
         DO k = kts,kte
            kflip = kte - k + kts              ! top down
            qvs1d(kflip) = qvsat(i,j,k)
            cfr1d(kflip) = cldfra(i,j,k)
            T1d(kflip) = t(i,j,k)
            P1d(kflip) = p(i,j,k)
            R1d(kflip) = rho(i,j,k)
            qc1d(kflip) = qc(i,j,k)
...
!     endif
     ...
     &                         debug_flag, qc1d, qi1d, qs1d, kts,kte)
         DO k = kts,kte
            kflip = kte - k + kts               ! flip back
            cldfra(i,j,kflip) = cfr1d(k)
            qc(i,j,kflip) = qc1d(k)
            qi(i,j,kflip) = qi1d(k)
ni_blck:    if (f_ni) then
               if (qi1d(k) .le. R1) then
                  nci = 0.
               else
                  nci = MAX(R2, ni(i,j,kflip)*R1d(k))  !-- # m^-3
                  lami = (cni1*nci/(R1d(k)*qi1d(k) ) )**rbmi
                  xDi = cni3/lami
                  if (xDi .lt. 20.E-6) then
                    lami = cni3/20.E-6
                  else if (xDi .gt. 300.E-6) then
                    lami = cni3/300.E-6
                  endif
                  nci = MIN(cni2*qi1d(k)*lami**bm_i, 250.D3/R1d(k) )  !- # kg^-1
               endif
               ni(i,j,kflip) = nci
            endif  ni_blck
         ENDDO
...
!.. k_tropo level is above 4km.
      ...
      !write(*,*)"tropopause height", "   k=", k
      !write(*,*)"tha2=",theta2," tha1=",theta1," ht1=",ht1," ht2=",ht2
