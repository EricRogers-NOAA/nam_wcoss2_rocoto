...
!
     ...
     &                      CLDCOV_V,PRSL,PRSLK,GT,GQ, VVEL,F_ICEC,     &
      ...
      real(kind=kind_phys),DIMENSION((ITE-ITS+1),LM+1) :: PRSI
!
...
!
      ...
      real (kind=kind_phys) :: fhr, solhr, SFCALBEDO((ITE-ITS+1)), SMX((ITE-ITS+1))

      integer, dimension(8) :: dp_start,dp_len
      logical, dimension(8) :: dp_day
...
!     
      ...
      dayparts: DO idaypart=1,ndayparts
       ...
       ! if ( GLON(I,J) >= 0.0 ) then
           XLON(1:im) = GLON((its+dps-1):(its+dps-1+im-1),J)
       ! else
       !    XLON(1) = GLON(I,J) + PI        ! if in -pi->+pi convert to 0->2pi
       ! endif

       XLAT(1:im)=GLAT((its+dps-1):(its+dps-1+im-1),J)
       SINLAT(1:im) = SIN ( XLAT(1:im) )
       COSLAT(1:im) = COS ( XLAT(1:im) )

       TSEA(1:im)=TSKIN((its+dps-1):(its+dps-1+im-1),J)
       TISFC(1:im)=TSKIN((its+dps-1):(its+dps-1+im-1),J)                  ! change later if necessary
       ZORL(1:im)=Z0((its+dps-1):(its+dps-1+im-1),J)*100.d0
       SNWDPH(1:im)=SI((its+dps-1):(its+dps-1+im-1),J)                    ! snwdph[mm]
       SNCOVR(1:im)=SNOWC((its+dps-1):(its+dps-1+im-1),J)
       SNOALB(1:im)=MXSNAL((its+dps-1):(its+dps-1+im-1),J)
       HPRIME_V(1:im)=STDH((its+dps-1):(its+dps-1+im-1),J)

       WHERE (SICE((its+dps-1):(its+dps-1+im-1),J).GT.0.5)                 ! slmsk - ocean  - 0
         SLMSK(1:im)= 2.0d0                    !         land   - 1
         FICE(1:im)=SICE((its+dps-1):(its+dps-1+im-1),J)                  ! change this later
       ELSEWHERE                            !         seaice - 2
         SLMSK(1:im)= 1.0d0-SM((its+dps-1):(its+dps-1+im-1),J)            !
...
!
      ...
      PRSI(1:im,1)=P8W((its+dps-1):(its+dps-1+im-1),1)/1000.                                ! [kPa]
!
      DO L=1,LM
        PRSI(1:im,L+1)=P8W((its+dps-1):(its+dps-1+im-1),L+1)/1000.                          ! (pressure on interface) [kPa]
        PRSL(1:im,L)=P_PHY((its+dps-1):(its+dps-1+im-1),L)/1000.                            ! (pressure on mid-layer) [kPa] 
        PRSLK(1:im,L)=(PRSL(1:im,L)*0.01d0)**(R/CP)
        RTvR(1:im)=1./(R*(Q((its+dps-1):(its+dps-1+im-1),J,L)*0.608+1.- &
                   CW((its+dps-1):(its+dps-1+im-1),J,L))*T((its+dps-1):(its+dps-1+im-1),J,L))
        VVEL(1:im,L)=OMGALF((its+dps-1):(its+dps-1+im-1),J,L)*1000.d0   &
                   *PRSL(1:im,L)*RTvR(1:im)            !not used
        GT(1:im,L)=T((its+dps-1):(its+dps-1+im-1),J,L)
        GQ(1:im,L)=Q((its+dps-1):(its+dps-1+im-1),J,L)
!
!--- GR1(:,:,1) - ozone
...
!
        ...
        ENDDO
!
        IF (NTOZ>0) GR1(1:im,l,1)=MAX(O3((its+dps-1):(its+dps-1+im-1),J,L),EPSO3)
!
        CLDCOV_V(1:im,L)=0.d0                     ! used for prognostic cloud
        TAUCLOUDS(1:im,L)=TAUTOTAL((its+dps-1):(its+dps-1+im-1),J,L)    ! CLOUD OPTICAL DEPTH (ICWP==-1)
        CLDF(1:im,L)=CLDFRA((its+dps-1):(its+dps-1+im-1),J,L)           ! CLOUD FRACTION

        GR1(1:im,L,3)=CW((its+dps-1):(its+dps-1+im-1),J,L)              ! total condensate
!
!----------
!
...
!----------
!
           GR1(1:im,L,4)=QC((its+dps-1):(its+dps-1+im-1),J,L)
           GR1(1:im,L,5)=QI((its+dps-1):(its+dps-1+im-1),J,L)
           GR1(1:im,L,6)=QS((its+dps-1):(its+dps-1+im-1),J,L)
           GR1(1:im,L,7)=QR((its+dps-1):(its+dps-1+im-1),J,L)
           GR1(1:im,L,8)=QG((its+dps-1):(its+dps-1+im-1),J,L)
           GR1(1:im,L,9)=NI((its+dps-1):(its+dps-1+im-1),J,L)
!----------
        ELSE  thompson_test
!----------
           F_ICEC(1:im,L)=F_ICE((its+dps-1):(its+dps-1+im-1),J,L)
           F_RAINC(1:im,L)=F_RAIN((its+dps-1):(its+dps-1+im-1),J,L)
           R_RIME(1:im,L)=F_RIMEF((its+dps-1):(its+dps-1+im-1),J,L)
!----------
        ENDIF  thompson_test
!----------
...
!
      ...
      CU_Bogus1: IF (CUCLD) THEN
       DO I=(its+dps-1),(its+dps-1+im-1)     !-- to be consistent with IM form "daypart" (ver. 44371, 20140815)
         ii = i-(its+dps-1)+1
...
!---
      ...
      SFCALBEDO(1:im) = ALBEDO((its+dps-1):(its+dps-1+im-1),J)
      SMX(1:im) = SM((its+dps-1):(its+dps-1+im-1),J)

!  --- ...  calling radiation driver

!      write(0,'("ibeg,jts,LENIVEC,im,dpd ",5i7)')(its+dps-1),jts,(ITE-ITS+1),im,dpd 
      call grrad_nmmb                                                   &
!!  ---  inputs:
           ...
           !  ALVSF1,ALNSF1,ALVWF1,ALNWF1,FACSF1,FACWF1,                &  ! processed inside grrad
             ...
             (its+dps-1),jts, (ITE-ITS+1), im, LM, dpd, MYPE, LPRNT, 0, 0,         &  ! jm dpd is true for day, false for night
...
!      do i=1,im
!      write(0,'("zap,i,j,hlw ",2i7,e25.15)')(its+dps-1+I-1),jts,HLW(i,1)
!      enddo

      COSZDG((its+dps-1):(its+dps-1+im-1),J) = COSZDG_V (1:im)
      CZMEAN((its+dps-1):(its+dps-1+im-1),J) = COSZEN_V (1:im)

      DO L=1,LM
        RLWTT((its+dps-1):(its+dps-1+im-1),J,L)=HLW(1:im,L)
        RSWTT((its+dps-1):(its+dps-1+im-1),J,L)=SWH(1:im,L)
      ENDDO
...
!=========================================================
      ...
      RLWIN((its+dps-1):(its+dps-1+im-1),J) =SFCFLW(1:im)%dnfxc
      RSWIN((its+dps-1):(its+dps-1+im-1),J) =SFCFSW(1:im)%dnfxc
      RSWOUT((its+dps-1):(its+dps-1+im-1),J)=SFCFSW(1:im)%upfxc
      RSWINC((its+dps-1):(its+dps-1+im-1),J)=SFCFSW(1:im)%dnfx0

      RLWTOA((its+dps-1):(its+dps-1+im-1),J)=TOPFLW(1:im)%upfxc
      RSWTOA((its+dps-1):(its+dps-1+im-1),J)=TOPFSW(1:im)%upfxc
...
!=================================================================
      ...
      IF (ICWP /= -1) THEN
         ...
         !===========================================================
            DO I=1,IM
              ARG_CW = MAXVAL( CW((its+dps-1+I-1),J,1:LM) )  !- for *total condensate*
         ...
         !=========================================================
            ...
            DO L=1,LM
               CLDFRA((its+dps-1):(its+dps-1+im-1),J,L)=CLDCOV_V(1:im,L)
               CSMID((its+dps-1):(its+dps-1+im-1),J,L)=CLDCOV_V(1:im,L)
            ENDDO

            CFRACL((its+dps-1):(its+dps-1+im-1),J)=CLDSA_V(1:im,1)
            CFRACM((its+dps-1):(its+dps-1+im-1),J)=CLDSA_V(1:im,2)
            CFRACH((its+dps-1):(its+dps-1+im-1),J)=CLDSA_V(1:im,3)
!
!@@@ To Do:  @@@
!@@@ Add CFRACT array to calculate the total cloud fraction, replace
!    the instantaneous cloud fraction in the post
!
            ACFRST((its+dps-1):(its+dps-1+im-1),J)=ACFRST((its+dps-1):(its+dps-1+im-1),J) + CLDSA_V(1:im,4)
            NCFRST((its+dps-1):(its+dps-1+im-1),J)=NCFRST((its+dps-1):(its+dps-1+im-1),J) + 1
!-- Added a time-averaged convective cloud fraction calculation
            WHERE (CU_cloud(1:im)) ACFRCV((its+dps-1):(its+dps-1+im-1),J)=ACFRCV((its+dps-1):(its+dps-1+im-1),J)+CLDSA_V(1:im,4)
            NCFRCV((its+dps-1):(its+dps-1+im-1),J)=NCFRCV((its+dps-1):(its+dps-1+im-1),J)+1
         ELSE
